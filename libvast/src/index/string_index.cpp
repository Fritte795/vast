...
bool string_index::serialize(caf::serializer& sink) const {
  if (!value_index::serialize(sink))
    return sink.apply(max_length_) && sink.apply(length_) && sink.apply(chars_);
  return true;
}

bool string_index::deserialize(caf::deserializer& source) {
  if (!value_index::deserialize(source))
    return source.apply(max_length_) && source.apply(length_)
           && source.apply(chars_);
  return true;
}
