...
bool inspect(caf::serializer& f, qualified_record_field& x) {
  ...
  return detail::inspect(f, layout_name, field_name, field_type);
}
...
bool inspect(caf::deserializer& f, qualified_record_field& x) {
  ...
  std::string field_name = {};
  legacy_type field_type = {};
  auto result = detail::inspect(f, layout_name, field_name, field_type);
  if (result)
